...
public class ProductCommandTest {
  ...
  void toCommandNameFail() {
    ...
    Iterator<ConstraintViolation<ProductCreateCommand>> iterator = violations.iterator();
    List<String> messages = new ArrayList<>();

    while (iterator.hasNext()) {
      ConstraintViolation<ProductCreateCommand> violation = iterator.next();
      messages.add(violation.getMessage());
    }

    assertThat(messages).contains("공백일 수 없습니다");
  }
